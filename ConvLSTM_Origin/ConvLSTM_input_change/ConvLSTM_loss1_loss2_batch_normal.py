...
import pandas as pd
import h5py
from keras.layers import ConvLSTM2D, Conv2D, Input, BatchNormalization, Flatten, Dense, Dropout
...
### Keras-based ConvLSTM Model with Enhanced Input Handling
...
def create_conv_lstm_model(timesteps, image_size, in_channel, out_channel):
    ...
    # ConvLSTM Layer with Batch Normalization
    ...
    conv_output = Conv2D(filters=pred_timesteps, kernel_size=(1, 1), activation='sigmoid')(conv_lstm)
...
if is_training:
    ...
    # Save the model
    model.save('convlstm_model_improved_batch_normal_64.h5')
    model.save_weights('convlstm_model_improved_weight_batch_normal_64.h5')
    print("Model saved.")

# Testing
if not is_training:
    model = tf.keras.models.load_model('convlstm_model_improved.h5', custom_objects={'total_loss': total_loss})
    loss_test = 0
    elapsed_time = 0
