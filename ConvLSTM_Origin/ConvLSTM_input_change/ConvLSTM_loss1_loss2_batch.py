...
def batch_creator_sequential(X, batch_size, dataset_length, start_index=0):
    batch_x = list()
    batch_y = list()
...
if is_training:
    for step in range(1, training_steps + 1):
        ...
        batch_x, batch_y, batch_ymap = batch_creator_sequential(X_train, batch_size, X_train.shape[0], start_idx)
        history = model.fit(batch_x, batch_y, batch_size=batch_size, verbose=0)

        # Validation
        batch_x_val, batch_y_val, batch_ymap_val = batch_creator_sequential(X_val, batch_size, X_val.shape[0], start_idx)
    ...
    # 모델 저장
    model.save('convlstm_model_improved_sequential.h5')
    model.save_weights('convlstm_model_improved_sequential_weight.h5')
    print("Model saved.")

# Test
if not is_training:
    model = tf.keras.models.load_model('convlstm_model_improved_sequential.h5', custom_objects={'total_loss': total_loss})
    loss_test = 0
    elapsed_time = 0
    ...
    for i in range(test_steps):
        ...
        batch_x_test, batch_y_test, batch_ymap_test = batch_creator_sequential(X_test, batch_size, X_test.shape[0], start_idx)
