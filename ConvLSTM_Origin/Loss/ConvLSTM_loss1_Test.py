...
logging.basicConfig(filename='test_log_loss1_loss2_improved_memory.txt', level=logging.INFO, format='%(asctime)s - %(message)s')
...
def find_neighbor(e):
    i = int(e / 32)
    j = e % 32  # Using modulo to find the correct column index

    # Determine the neighboring rows
    if i == 0:
        i_nei = [0, 1]
    elif i == 31:
        i_nei = [30, 31]
    ...
    # Determine the neighboring columns
    if j == 0:
        j_nei = [0, 1]
    elif j == 31:
        j_nei = [30, 31]
