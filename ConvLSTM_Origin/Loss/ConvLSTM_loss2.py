...
def batch_creator(X, batch_size, dataset_length):
    ...
    batch_ymap = np.zeros((batch_size, output_size))  # 특정 station을 사용하도록 마스크 적용
...
def compile_model(model, neighbors_dict):
    model.compile(
        ...
        loss=lambda y_true, y_pred: total_loss(y_true, y_pred, neighbors_dict)  # loss2만 사용
    )
...
if is_training:
    ...
    # 모델 저장
    model.save('convlstm_model_loss2_only.h5')
    model.save_weights('convlstm_model_loss2_only_weight.h5')
    print("Model saved.")

# Test
if not is_training:
    model = tf.keras.models.load_model('convlstm_model_loss2_only.h5', custom_objects={'total_loss': total_loss})
    loss_test = 0
    elapsed_time = 0
