...
def compute_loss1(y_true, y_pred):
    ...
    return tf.sqrt(mse_loss(y_true, y_pred) * loss_ratio)
...
if not is_training:
    ...
    for i in range(test_steps):
        ...
        loss_value = sqrt(tf.keras.losses.mean_squared_error(inv_y, inv_yhat).numpy() * loss_ratio)
