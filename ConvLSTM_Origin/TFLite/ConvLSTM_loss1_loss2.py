...
if is_training:
    ...
    # 모델 저장
    model.save('convlstm_model_loss1_loss2.h5')
    model.save_weights('convlstm_model_loss1_loss2_weight.h5')
    print("Model saved.")

    # Step 1: TFLite 모델로 변환
    ...
    # TFLite 모델 저장
    with open('convlstm_model_loss1_loss2.tflite', 'wb') as f:
        f.write(tflite_model)
    print("TFLite Model saved.")

# Test with TFLite model
def test_tflite_model(tflite_model_path, X_test, scaler):
    # TFLite 인터프리터 로드
    ...
    for i in range(test_steps):
        ...
        # TFLite 모델에 입력을 설정
        interpreter.set_tensor(input_details[0]['index'], batch_x_test)
...
if not is_training:
    test_tflite_model('convlstm_model_loss1_loss2.tflite', X_test, scaler)
